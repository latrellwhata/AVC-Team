...
// other Varibles
		int debug = 1; // Turns on all the print statements
...
// For Vertiacal Right Line
		...
		int num_white_pixels_tot = num_white_pixels_H+num_white_pixels_VL+num_white_pixels_VR;	
...
// makes sure motor values are not too low
	...
	// setting Motor speed based on Image	
  		...
  		if (third == false &&  num_white_pixels_H>300 && num_white_pixels_VL>70 && num_white_pixels_VR>70 ){
				...
				sleep1(0,500000);
				third = true;
				printf("FULL WHITE RESESERED FULL WHITE RESISTERED FULL WHITE REGISTERED");
				goto start;
		}
  		
  		
  		if (third == true){ // Quadrent 3
				printf("FULL WHITE REGISTERED");
			
				if (num_white_pixels_VL > 50  && num_white_pixels_H >300 && num_white_pixels_VR>50 ){ //this is at a turn option
					if (debug == 1){
				...
				if (num_white_pixels_VR == 0 && num_white_pixels_H > 75 && num_white_pixels_VL >40){
					v1 = -180;
					v2 =  200;
					set_motor(1,v1);
					set_motor(2,v2);
				...
				if (num_white_pixels_VL == 0 && num_white_pixels_H > 75 && num_white_pixels_VR >40){ //this is at must right turn
		...
		if (num_white_pixels_H>3 ){ //if this is not above 0 then we dont have a siginal
			if (debug == 1){
			...
			//	}
				...
				if (third == true && num_white_pixels_tot==0) { // does a U-Turn
				v1 =-180;
				v2 = 200;
				set_motor(1, v1);
				set_motor(2, v2);
			...
			num_white_pixels_VL =0;
			num_white_pixels_VR =0;
			num_white_pixels_tot=0;
